...
As such, the neural network output is simply $o(\mathbf{x}_{in},\mathbf{w})$. The loss function is an additional step computing a metric function on the output and supervised output labels of the training data. The mean-squared loss (2-norm) is a simple example: $\lVert o(\mathbf{x}_{in},\mathbf{w}) - l \rVert_{2}$, where weight vector $\mathbf{w}$ is known to produce a scalar output $l$, and the 2-norm (mean squared loss) is only really sensible in the limit of multiple outputs (so that $\mathbf{o}$ and $\mathbf{l}$ become vectors)
